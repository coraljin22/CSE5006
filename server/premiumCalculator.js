...
/**
 * Calculate Lifetime Health Cover loading percentage.
 *
 * Rules:
 * - Hospital cover must not be None.
 * - Cover history Yes = 0%.
 * - Cover history Not sure = 0%, but show a warning.
 * - Cover history No:
 *      if age > 30, loading = (age - 30) * 2%
 *      otherwise loading = 0%.
 */
function calculateLhcLoading(age, coverHistory, hospitalCover) {
  ...
  if (coverHistory === "Not sure") {
    return {
      loadingPercentage: 0,
      warning:
        "Hospital cover history is not confirmed. No LHC loading has been applied.",
    };
  }
...
/**
 * Calculate a complete health cover quote.
 */
function calculatePremium(quote) {
  ...
  const applicant1HospitalCost =
    hospitalBasePrice * (1 + applicant1Lhc.loadingPercentage / 100);
  ...
  if (adultCount === 2) {
    ...
    applicant2HospitalCost =
      hospitalBasePrice * (1 + applicant2Lhc.loadingPercentage / 100);
  }
  ...
  const familyFee = cover_type === "Family" ? FAMILY_FEE : 0;
  ...
  const discountPercentage =
    payment_frequency === "Yearly" ? annual_discount : 0;

  const discountAmount =
    yearlyBeforeDiscount * (discountPercentage / 100);

  const yearlyPremium = yearlyBeforeDiscount - discountAmount;

  const warnings = [];

  if (applicant1Lhc.warning) {
    warnings.push(`Applicant 1: ${applicant1Lhc.warning}`);
  }

  if (applicant2Lhc.warning) {
    warnings.push(`Applicant 2: ${applicant2Lhc.warning}`);
  }
